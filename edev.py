...
async def getKeycodeAsync() -> list[str]:
    ...
    async for event in dev.async_read_loop():
        print(f"Received event: type={event.type}, code={event.code}, value={event.value}")
        if event.type == ecodes.EV_KEY:
            categorizedEvent = categorize(event) # categorizes events based on types such as: EV_KEY, EV_REL, EV_ABS, EV_MSC, EV_LED, EV_SND, EV_REP, EV_FF, EV_PWR, EV_FF_STATUS, EV_MAX
